...
LLM agents in permission-less mode. You should consider your own risk tolerance
before trusting this setup though.

Specifically:

- Using [apple/container] rather than a runtime like docker gives VM level
isolation to the container
- The init-firewall.sh script prevents most internet access from the container
...
by the virtualization framework. You still have to be careful with any keys you
give processes inside the container though — as soon as you log in to Claude
there's at least one valuable secret in the environment: your Claude session
token.
